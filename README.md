...
п59: Реализую поддержку отправки изображений в Telegram через connectors/telegram.py, если указан URL в post['image'].
п60: Добавлю graceful shutdown в main.py для обработки SIGTERM и завершения текущего медиа-такта перед остановкой.
п61: Расширю publisher.py для сохранения статистики вовлечённости (просмотры, действия) в /data/stats.csv, используя заглушку для Telegram API.

Изображения: Поддерживаются в Telegram-постах.
Graceful shutdown: Обеспечивает корректное завершение.
Статистика: Сохраняется в /data/stats.csv для анализа вовлечённости.
Мультиязычность: Поддерживается через config.yaml.
Профили журналистов: Эмодзи для casual_host в ai_journalist.py.
Предотвращение дублирования: Через векторы в utils.py.
